...
const PhotoGrid = () => {
  ...
  return <section id="photo-grid" className="py-20 px-4 relative overflow-hidden">
      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="grid grid-cols-3 gap-6 max-w-4xl mx-auto">
          {photos.map((photo, index) => <div key={index} className={`photo-card group relative overflow-hidden rounded-2xl aspect-square transition-all duration-1000 delay-${index * 100} ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} style={{
          transform: `perspective(1000px) rotateX(${(mousePosition.y - window.innerHeight / 2) * 0.01}deg) rotateY(${(mousePosition.x - window.innerWidth / 2) * 0.01}deg)`
              ...
              <div className="photo-card-inner w-full h-full relative preserve-3d">
                <div className="photo-front absolute inset-0 backface-hidden">
                  <img src={`https://images.unsplash.com/${photo}?w=400&h=400&fit=crop`} alt={`Memory ${index + 1}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
