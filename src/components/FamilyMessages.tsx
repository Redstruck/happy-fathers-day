      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {messages.map((msg, index) => (
            <Card
              key={index}
              className={`glassmorphism border-0 transition-all duration-1000 delay-${index * 150} hover:scale-105 magnetic-hover group ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            >
              <CardContent className="p-6 relative overflow-hidden">
                <div className="ripple absolute inset-0"></div>
                <div className={`w-16 h-16 rounded-full bg-gradient-to-r ${msg.color.replace('/20', '/50')} flex items-center justify-center mb-4 mx-auto neumorphism group-hover:scale-110 transition-transform duration-300`}>
