...
const FamilyMessages = () => {
  ...
  return <section id="family-messages" className="py-20 px-4 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
      {/* Background particles */}
      <div className="absolute inset-0 overflow-hidden">
        {Array.from({
        length: 10
      }).map((_, i) => <div key={i} className="absolute w-1 h-1 bg-blue-400/30 rounded-full animate-float-particle" style={{
      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {messages.map((msg, index) => <Card key={index} className={`glassmorphism border-0 transition-all duration-1000 delay-${index * 150} hover:scale-105 magnetic-hover group ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
